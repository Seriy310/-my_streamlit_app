...
# Навігація на праву панель для переходу до "Кредит"
credit_section = st.sidebar.empty()  # Окрема зона для відображення іконки або кнопки

# Виведення тексту на панелі з можливістю навести курсор
...
# Якщо користувач натискає на "Кредит" в бічній панелі, він потрапляє на історію оплат
if st.sidebar.button("Перейти до історії оплат"):
    st.sidebar.markdown("### Історія оплат")
    # Поля для введення оплат
    ...
    # Якщо не натискають на кнопку, відображається основний вміст
